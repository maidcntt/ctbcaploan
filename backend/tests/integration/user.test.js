...
const { admin, userOne, userTwo, adminWhoCreateUser, insertUsers } = require('../fixtures/user.fixture');
const { userOneAccessToken, adminAccessToken, adminWhoCreateUserAccessToken } = require('../fixtures/token.fixture');
...
describe('User routes', () => {
  describe('POST /v1/users', () => {
    ...
    test('should return 201 and successfully create new user if data is ok', async () => {
      await insertUsers([adminWhoCreateUser]);

      const res = await request(app)
        .post('/v1/users')
        .set('Authorization', `Bearer ${adminWhoCreateUserAccessToken}`)
        .send(newUser)
  ...
  describe('GET /v1/users', () => {
    ...
    test('should correctly apply filter on role field', async () => {
      ...
      expect(res.body.rows[0].id).toBe(userOne.id);
      expect(res.body.rows[1].id).toBe(userTwo.id);
    });
    ...
    test('should correctly sort the returned array if descending sort param is specified', async () => {
      ...
      expect(res.body.rows).toHaveLength(3);
      expect(res.body.rows[0].id).toBe(admin.id);
      expect(res.body.rows[1].id).toBe(userOne.id);
      expect(res.body.rows[2].id).toBe(userTwo.id);
    });
    ...
    test('should correctly sort the returned array if ascending sort param is specified', async () => {
      ...
      expect(res.body.rows[0].id).toBe(userOne.id);
      expect(res.body.rows[1].id).toBe(userTwo.id);
      expect(res.body.rows[2].id).toBe(admin.id);
    });
    ...
    test('should return the correct page if page and limit params are specified', async () => {
      ...
      expect(res.body.rows).toHaveLength(1);
      expect(res.body.rows[0].id).toBe(userTwo.id);
    });
  });
  ...
  describe('GET /v1/users/:user_id', () => {
    ...
    test('should return 400 error if user_id is not a valid id', async () => {
      await insertUsers([admin]);
